...
for intRunPrePro=[1]
    ...
    strRezFile = fullfile(strPathEphys,'rez2.mat');
    if ~exist(strRezFile,'file')
        strRezFile = fullfile(strPathEphys,'rez.mat');
    end
    sLoad = load(strRezFile);
    sRez = sLoad.rez;
    ...
    %remove nans
    strUseSyncFieldOn = 'vecStimOnTime';
    strUseSyncFieldOff = 'vecStimOffTime';
    for intStim=1:numel(cellStim)
        if ~isfield(cellStim{intStim}.structEP,strUseSyncFieldOn)
            strUseSyncFieldOn = 'vecStimOnNI';
            strUseSyncFieldOff = 'vecStimOffNI';
        end
        matStimOnOff = [cellStim{intStim}.structEP.(strUseSyncFieldOn);cellStim{intStim}.structEP.(strUseSyncFieldOff)]';
        %remove nans
    ...
    if ~boolOnlyJson
        ...
        parfor intCluster=1:intClustNum
            ...
            for intStim=1:numel(cellStim)
                ...
                %get responsiveness
                dblDur = nanmedian(diff(cellStim{intStim}.structEP.(strUseSyncFieldOn)))
                [dblZETA,vecLatencies,sZETA] = getZeta(vecSpikeTimes,matStimOnOff,dblDur,50,0,0);
                %sZETA=[];
