function [sFigRP,sRP] = RP_genGUI(varargin)
	...
	for intPathButton=1:intPathNum
		...
		sFigRP.(strOutFieldButton) = uicontrol(sFigRP.ptrPanelPaths,'Style','pushbutton','FontSize',11,...
			...
			'Tooltip',sprintf('Set path to root directory for %s',strPathName),...
	...
	vecLocVariablesButton = [vecLocSetPath(1) vecLocSetPath(2)-30 120 25];
		sFigRP.ptrButtonCompileLibrary = uicontrol(sFigRP.ptrPanelPaths,'Style','pushbutton','FontSize',11,...
		'String','Edit variables',...
		'Position',vecLocVariablesButton,...
		'Tooltip','Set recording variables and meta data',...
		'Callback',@ptrButtonEditVariables_Callback);

	%compile button
	vecLocCompileButton = [vecLocVariablesButton(1)+vecLocVariablesButton(3)+5 vecLocVariablesButton(2) 220 25];
	sFigRP.ptrButtonCompileLibrary = uicontrol(sFigRP.ptrPanelPaths,'Style','pushbutton','FontSize',11,...
		'String','Compile data library',...
		'Position',vecLocCompileButton,...
		'Tooltip','Search data paths for data',...
	...
	sFigRP.ptrButtonDoClustering = uicontrol(ptrMainGUI,'Style','pushbutton','FontSize',11,...
		...
		'Tooltip',sprintf('Cluster spikes with external sorter (%s)',strSorter),...
	...
	sFigRP.ptrButtonSetLabels = uicontrol(ptrMainGUI,'Style','pushbutton','FontSize',11,...
		...
		'Tooltip','Align & combine data from multiple sources to a single clock',...
	...
	sFigRP.ptrButtonAutopilot = uicontrol(ptrMainGUI,'Style','pushbutton','FontSize',11,...
		...
		'Tooltip','Assign Allen Brain Atlas areas to putative units',...
	...
	sFigRP.ptrButtonCheckResults = uicontrol(ptrMainGUI,'Style','pushbutton','FontSize',11,...
		...
		'Tooltip','Check responsiveness and location of all units',...
	...
	sFigRP.ptrButtonExportData = uicontrol(ptrMainGUI,'Style','pushbutton','FontSize',11,...
		...
		'Tooltip','Export synchronized data & area-assigned units to aggregate output file',...
	...
	function ptrButtonSetPath_Callback(hObject, eventdata,strPathName)
		...
		if ischar(strNewPath) && ~isempty(strNewPath) && exist(strNewPath','dir')
			...
			sFigRP.(strTextField).Tooltip = sFigRP.(strTextField).String;
		else
			return;
		end
		...
		if strcmpi(strPathName,'Temp')
			...
			sFigRP.ptrTextTempSpace.Tooltip = sFigRP.(strTextField).String;
		end
	end
			...
			for intFileIdx=1:numel(vecRunFiles)
				try
					...
					if ~isempty(sClustered)
						...
						sFigRP.sPointers(intFile).Clustered.Tooltip = ['Clustered data at: ' sClustered.folder];
						drawnow;
					end
				catch ME
					dispErr(ME);
