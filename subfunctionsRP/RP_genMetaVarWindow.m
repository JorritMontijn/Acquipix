function sMetaVar = RP_genMetaVarWindow()
	...
	function MV_genFig()
		...
		%define size
		dblLineHeight = 30;
		dblTotHeight = dblLineHeight*(intFieldNum + 6.5);
		vecPosGUI = [0,0,500,dblTotHeight];
		sFigMV.ptrMainGUI.Position = vecPosGUI;
		
		%help button
		vecLocHelp = [vecPosGUI(3)-10-50 dblTotHeight-dblLineHeight 50 25];
		sFigMV.ptrButtonLoad = uicontrol(sFigMV.ptrMainGUI,'Style','pushbutton','FontSize',11,...
			'String',sprintf('Help'),...
			'Position',vecLocHelp,...
			strTooltipField,sprintf('Show help pop-up'),...
			'Callback',@ptrButtonHelp_Callback);
		
		%load & save buttons
		vecLocLoad = [10 dblTotHeight-2*dblLineHeight 50 25];
	...
	function ptrButtonHelp_Callback(handles,eventdata)
		strMsg = {'All fields here will be saved as metadata in a json file after export.',...
				'You can add any field with string data, but note the following:',...
				'Fieldnames starting with niCh, e.g., niCh0, niCh1, etc can only take',...
				'three types of values:',...
				'',...
				'1) sync',...
				'   This means the channel is the synchronization pulse-channel.',...
				'   NB: make sure this matches the information in the nidq meta file!',...
				'',...
				'2) onset',...
				'   This means these pulses will be used to synchronize stimulus onsets.',...
				'   Example: feedback photodiode voltages that pick up screen onsets',...
				'',...
				'3) data-processor callback function-names, e.g.: @PP_GetRunSpeed',...
				'   This will feed the data in the specified NI channel to this function.',...
				'   It will create a field with the function''s name (sAP.PP_GetRunSpeed)',...
				'   that contains the pre-processed data. You will have to create a dummy',...
				'   function that feeds back its input if you wish to save the raw NI data.',...
				'   The function saves only the first output, but this can be a structure.',...
				'',...
				'For more information: read the fabulous manual (RTFM)'};
		h=msgbox(strMsg,'MetaVars instructions');
		h.Children(end).Children.FontSize=10;
		h.Position=[300 300 350 300];
		movegui(h,'center');
	end
end