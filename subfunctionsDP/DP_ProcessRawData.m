function rez = DP_ProcessRawData(matData,sChanMap)
...
%
%Based on Kilosort2's preprocess_data_sub.m
...
xcoords = sChanMap.xcoords;
ycoords = sChanMap.ycoords;
NchanTOTdefault = sChanMap.NchanTOTdefault;

	% = loadChanMap(ops.chanMap); % function to load channel map file
ops.NchanTOT = getOr(ops, 'NchanTOT', NchanTOTdefault); % if NchanTOT was left empty, then overwrite with the default

igood = DP_CullChannels(matData, ops, vecChanMap);


    vecChanMap = vecChanMap(igood); %it's enough to remove bad channels from the channel map, which treats them as if they are dead

    xcoords = xcoords(igood); % removes coordinates of bad channels
    ycoords = ycoords(igood);
