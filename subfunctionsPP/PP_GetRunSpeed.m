function sRunSpeed = PP_GetRunSpeed(vecData,sMetaNI)
    ...
    if ischar(dblSampRateNI)
        dblSampRateNI = str2double(dblSampRateNI);
    end
    ...
    try
        %try gpu filtering
        gVecRunStep = gpuArray(vecRunStep);
        gVecFilt = gpuArray(vecFilt);
        vecMean = conv(gVecRunStep,gVecFilt,'same');
    catch
        vecMean = conv(vecRunStep,vecFilt,'same');
    end
