...
if ~exist('sStimParamsSettings','var') || isempty(sStimParamsSettings)
	%parameters
	sStimParamsSettings.strStimType = 'OptoWhiskerStim';
	...
	sStimParamsSettings.dblPulseVoltage = 2;%volts
	sStimParamsSettings.dblSamplingRate = 10000;%Hz
	sStimParamsSettings.intUseDaqDevice = 1; %ID of DAQ device
	sStimParamsSettings.strPortOut_opto = 'ao0'; %output port of DAQ device for opto
	sStimParamsSettings.strPortOut_whisker = 'ao1'; %output port of DAQ device for whisker
	
elseif ~strcmpi(sStimParamsSettings.strStimType,'OptoWhiskerStim')
...
cellPulseDelay = cell(1,intTrialNum);
...
for intTrial=1:intTrialNum
	...
	for intDelayType=1:numel(vecShuffDelay)
		...
		%vecPulses1 = repmat(vecOnePulse,[Delay_number 1]);
		vecWait = false(round(dblSamplingRate*dblPulseWaitSignal),1);
		vecData1 = cat(1,vecData1,vecOnePulse1,vecWait);
		
		%whisker data
		vecDelayData = false(round(vecShuffDelay(intDelayType)*dblSamplingRate),1);
		ITIshortened = false(round(vecPulseITI*dblSamplingRate-vecShuffDelay(intDelayType)*dblSamplingRate),1); %subtract delay at the end
		vecOnePulse2 = cat(1,vecDelayData,true(round(vecPulseDur*dblSamplingRate),1),ITIshortened);
		%vecPulses2 = repmat(vecOnePulse,[Delay_number 1]);
		vecWait = false(round(dblSamplingRate*dblPulseWaitSignal),1);
		vecData2 = cat(1,vecData2,vecOnePulse2,vecWait);
	end
	
	
	vecPulseVolt(intTrial) = dblPulseVoltage;
	cellPulseData{intTrial} = cat(2,vecData1,vecData2);
	cellPulseDelay{intTrial} = vecShuffDelay;
	cellPulseITI{intTrial} = vecPulseITI;
	cellPulseDur{intTrial} = vecPulseDur;
end
...
if boolUseNI && sStimParamsSettings.intUseDaqDevice > 0
	...
	%add opto output channels
	[chOut0,dblIdx0] = addAnalogOutputChannel(objDAQOut, strID, strPortOut_opto, 'Voltage');
	
	%add whisker output channels
	[chOut1,dblIdx1] = addAnalogOutputChannel(objDAQOut, strID, strPortOut_whisker, 'Voltage');
	
	%% set spritzer off
	dblStartT = 0.1;
	%queueOutputData(objDAQOut,repmat([0 0],[ceil(objDAQOut.Rate*dblStartT) 1]));
	queueOutputData(objDAQOut,zeros([ceil(objDAQOut.Rate*dblStartT) 2]));
...
structEP.vecPulseDur_opto = vecPulseDur;%duration of opto and whisker stimulus (secs), at the moment same duration only
...
try
	...
	for intTrial = 1:intTrialNum
		...
		cellPulseDelay_Temp = cellPulseDelay(1:(intTrial-1));
		...
		save(fullfile(strTempDir,[strFilename '_Temp']),...
			'vecPulseVolt_Temp','cellPulseData_Temp','cellPulseDelay_Temp','cellPulseITI_Temp','cellPulseDur_Temp','vecStimOnNI_Temp','vecStimOffNI_Temp');
		...
		if boolUseNI && sStimParamsSettings.intUseDaqDevice > 0
			...
			stop(objDAQOut);
			opto_data = matData(:,2);
			whisker_data = matData(:,1);
			queueOutputData(objDAQOut,[opto_data whisker_data]);   %swap in case you want opto to go first
			prepare(objDAQOut);
		end
	...
	structEP.cellPulseDelay = cellPulseDelay(1:intTrial);
